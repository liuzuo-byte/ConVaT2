...
from utils import count_sliding_window, get_device, grouper, logger, sample_gt,metrics, sliding_window
...
img = applyPCA(img,numComponents=103)#在main函数里降维
...
train_dataset1 = HyperX(img, train_gt, **hyperparams)

train_loader1 = data.DataLoader(train_dataset1,
...
model = ViT_Encoder(image_size = 15,patch_size = 3,dim = 1024,depth = 2,heads = 8,
                        mlp_dim = 64,channels = 103,dropout = 0.1,emb_dropout = 0.1).cuda()

def L2_Norm(data):#估计是用来正则化特征
    norm=np.linalg.norm(data, ord=2)
    return truediv(data,norm)

model.load_state_dict(torch.load('/home/lmm/liuzuo/VAE_BYOL_HSI/checkpoints/VAE_BYOL_param/0.99UP'))
model = model.to(args.cuda)
classifier = nn.Linear(in_features=1024, out_features=10, bias=True).to(args.device)
...
for i, (data,_,label) in enumerate(tqdm(train_loader1)):
...
# Encoder_Features = L2_Norm(Encoder_Features)#868*1024（961个样本，每个样本特征128维）
Encoder_Features = np.array(Encoder_Features)#不需要正则化，直接去分类，正则化的话精度会降低，可能画图也不需要正则化
Label = np.array(Label)#868*1024
...
# plt.scatter(X_tsne[:, 0], X_tsne[:, 1],c=Label,cmap='rainbow')#c为标签可代表散点的颜色，标签是连续的，为了区分颜色使用cmap
# plt.legend()
# plt.savefig('003'+'.png')#保存图片路径
# plt.show()
...
train_dataset2 = TensorDataset(Encoder_Features, Label)
# print(train_dataset2[0:5])
...
# define optimizer
optimizer = torch.optim.Adam(classifier.parameters(), lr = 0.001)#lr取0.01，精度会高两个点
...
for e in tqdm(range(1, EPOCH + 1), desc="Training the network"):
    ...
    for batch_idx, (data,label) in enumerate(train_loader2):
        
        data, label = data.to(args.cuda), label.to(args.cuda)
        
        classifier.zero_grad()

        pred = classifier.forward(data)
